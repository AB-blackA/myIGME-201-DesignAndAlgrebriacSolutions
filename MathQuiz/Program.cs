...
// Class: Program
// Author: David Schuh, Andrew Black 
...
static class Program
{
    ...
    start:
        ...
        //inform user of timer
        // ask difficulty level until they enter a valid response
        do
        {
            Console.Write("There is a timer of 5 seconds per question. Choose your Difficulty level (easy, medium, hard)-> ");
        ...
        for (nCntr = 1; nCntr <= nQuestions; ++nCntr)
        {
            ...
            do
            {
                ...
                // reset user state of timing out
                timeOut = false;

                // make new timer with 5 seconds to operate
                ...
                // only ask for an int if they arent timed out
                if (!bValid && !timeOut)
                {
            ...
            else if (timeOut)
            {
                ...
                Console.WriteLine("I'm sorry {0}, you took too long to answer. The answer is {1}", myName, nAnswer);
            }
    ...
    {
        Console.WriteLine("\nYour times up! Please press Enter to continue");
        timeOut = true;
        timer.Stop();
    }
